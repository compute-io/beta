...
function beta( x, y, options ) {
	...
	// Handle case where one or more arguments is not a supported type...
	xFlg = !xType;
	yFlg = !yType;
	if (
		xFlg && yFlg ||
	...
	// Note that the following accounts for unsupported types (e.g., `null`, `undefined`, `boolean`, etc) being paired with accepted argument types...
	else {
		len = 0;
	}
